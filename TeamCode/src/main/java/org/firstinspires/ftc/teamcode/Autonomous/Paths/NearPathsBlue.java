...
/** Blue-alliance near-side tunnel paths. Copy final values back here when tuning is done. */
public class NearPathsBlue extends NearPaths {

    private static final double GOAL_X = 12, GOAL_Y = 135;
    private final ShootPathBuilder shootBuilder;

    public NearPathsBlue(Follower follower) {
        ...
        Intake1 = follower.pathBuilder().addPath(
                new BezierCurve(new Pose(45, 95), new Pose(68, 78), new Pose(17, 90))
        ).setLinearHeadingInterpolation(Math.toRadians(137), Math.toRadians(180)).build();

        Gate1 = follower.pathBuilder().addPath(
                new BezierCurve(new Pose(17, 90), new Pose(35, 83), new Pose(5, 78))
        ).setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(90)).build();

        Gate1V2 = follower.pathBuilder().addPath(
                new BezierCurve(new Pose(15, 86), new Pose(39, 85), new Pose(14, 74))
        ).setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(90)).build();

        Shoot2 = follower.pathBuilder().addPath(
                new BezierLine(new Pose(9, 78), new Pose(45, 95))
        ...
        Gate2 = follower.pathBuilder().addPath(
                new BezierCurve(new Pose(15, 60), new Pose(50, 60), new Pose(9, 78))
        ).setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(90)).build();
        ...
        Intake3Gate = follower.pathBuilder().addPath(
                new BezierCurve(new Pose(45, 95), new Pose(63, 28), new Pose(16, 40))
    ...
    @Override public ShootPathBuilder.Result buildShoot2()     { return shootBuilder.build(new Pose(9, 78), new Pose(45, 95)); }
    @Override public ShootPathBuilder.Result buildShoot3()     { return shootBuilder.build(new Pose(15, 60), new Pose(23, 60), new Pose(45, 95)); }
    @Override public ShootPathBuilder.Result buildShoot4()     { return shootBuilder.build(new Pose(25, 36), new Pose(45, 95)); }
    @Override public ShootPathBuilder.Result buildGateShoot4() { return shootBuilder.build(new Pose(8, 62), new Pose(45, 34), new Pose(45, 95)); }
}
