...
@Autonomous
public class BlueNearAuto extends AbstractOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        SequentialCommandGroup auto = SequentialCommandGroup.getBuilder()
                ...
                //.add(new AimCommand(drivebase, limeLight, telemetry, follower, blueNearPaths.GOAL_X, blueNearPaths.GOAL_Y))
                .add(new TimedShootCommand(shooter, intake, 0.75, telemetry, 1100, servoGate, 0.95, 0.45))
                .add(new PedroDriveToCommand(follower, blueNearPaths.ShootToSet, 2, telemetry))
                .add(ParallelCommandGroup.getBuilder()
                        .add(new IntakeCommand(intake, 1.0, telemetry, servoGate))
                        .add(new PedroDriveToCommand(follower, blueNearPaths.SetToBase, 2, telemetry))
                        .build()
                )
                .add(ParallelCommandGroup.getBuilder()
                        .add(new IntakeCommand(intake, 1.0, telemetry, servoGate))
                        .add(new PedroDriveToCommand(follower, blueNearPaths.BaseToShoot, 2, telemetry))
                        .add(new TimedShootCommand(shooter, intake, 0.75, telemetry, 1100, servoGate, 0.95, 0.45))
                        .build()
                )
